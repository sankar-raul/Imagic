...
export default function Header() {
  ...
  return (
    ...
    >
        ...
        >
          <div
            className={`w-8 h-0.5 bg-gray-800 rounded-full absolute transition-all duration-300 ${
              navDisplay === "translate-y-0" ? "-rotate-45" : "top-0"
            }`}
          ></div>
          <div
            className={`w-8 h-0.5 bg-gray-800 rounded-full transition-all duration-300 ${
              navDisplay === "translate-y-0" ? "opacity-0" : "opacity-100"
            }`}
          ></div>
          <div
            className={`w-8 h-0.5 bg-gray-800 rounded-full absolute transition-all duration-300 ${
              navDisplay === "translate-y-0" ? "rotate-45" : "bottom-0"
            }`}
          ></div>
        </button>
      </div>
      ...
      {createPortal(
        <div
          className={`${navDisplay} max-h-dvh flex-col lg:hidden duration-300 fixed left-0 top-0 h-full w-full z-50`}
        >
          {/* Mobile Backdrop */}
          <div
            onClick={handleNavOpen}
          ...
          <nav className="w-full bg-white overflow-y-auto max-h-full rounded-none px-6 py-8 relative h-max">
            <button
              onClick={handleNavOpen}
              className="flex flex-col bg-white z-6 p-6 sticky top-0 ml-auto gap-1.5 w-8 h-8 rounded-sm items-center justify-center"
              aria-label="Toggle menu"
            >
              <div
                className={`w-8 h-0.5 bg-gray-800 rounded-full absolute transition-all duration-300 ${
              ...
              {navData.menuItems.map((item, index) => (
                <li className="relative group" key={index}>
                  {item.type === "link" ? (
                    <NavLink
                      to={item.href || "/"}
                      onClick={() => setNavDisplay("-translate-y-full")}
                    ...
                    <>
                      ...
                      >
                        {item.label}
                        <span className="ml-1 text-xs transition-transform duration-300 lg:inline-block origin-center group-hover/navbutton:rotate-180">
                          ▼
                        </span>
                      </button>
