      ...
      items: [
        {
          ...
          items: [
            { label: "Graphic Design Course In Kolkata", href: "/course/graphic-design-kolkata" },
            { label: "Certificate Course In Adobe InDesign", href: "/course/certificate-course-in-adobe-indesign" },
            { label: "Certificate Course In Adobe Lightroom", href: "/course/certificate-course-in-adobe-lightroom" }
          ]
        },
        {
          label: "Video Editing Course",
          items: [
            { label: "FCP Video Editing & VFX Course", href: "/course/fcp-video-editing-vfx-course" },
            { label: "Professional Video Editing Course ", href: "/course/professional-video-editing-in-kolkata" },
            { label: "After Effect Course", href: "/course/certificate-course-in-after-effects" },
            { label: "FCP Video Editing Course", href: "/course/fcp-video-editing" }
          ]
        },
        {
          label: "Digital Marketing Courses",
          items: [
            { label: "Digital Marketing Course In Kolkata", href: "/course/digital-marketing-course-in-kolkata" },
          ]
        },
        {
          label: "NSOU Diploma Courses",
          items: [
            { label: "NSOU Video Editing Diploma", href: "/course/video-editing-diploma-course" },
            { label: "NSOU Digital Marketing Diploma ", href: "/course/digital-marketing-diploma-course" },
            { label: "NSOU Graphic Design Diploma", href: "/course/graphic-design-diploma-course" },
          ]
        }
      ]
    },
...
export default function Navbar() {
  ...
  return (
    <header className={`sticky top-0 flex justify-between items-center py-3 px-6 md:px-12 lg:px-16 gap-4 z-50 shadow-sm transition-all duration-500 ${
      !isScrolled 
        ? 'lg:w-max lg:mx-auto lg:bg-white/80 lg:backdrop-blur-2xl lg:rounded-full md:translate-y-2' 
                ...
                {item.type === "link" ? (
                  ...
                  <>
                    <button
                      ...
                      className="w-full lg:w-auto flex items-center justify-between lg:justify-center px-4 py-3 lg:py-4 text-gray-800 hover:text-gray-900 font-medium text-base transition-colors relative lg:after:content-[''] lg:after:absolute lg:after:bottom-0 lg:after:left-1/2 lg:after:-translate-x-1/2 lg:after:w-0 lg:after:h-0.5 lg:after:bg-purple-600 lg:hover:after:w-3/4 lg:after:transition-all lg:after:duration-300"
                    >
                      {item.label}
                      <span className="ml-1 text-xs transition-transform duration-300 lg:inline-block" style={{ transform: expandDetails ? 'rotate(180deg)' : 'rotate(0)' }}>▼</span>
                    </button>
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        {item.items?.map((category, catIndex) => (
                          <div key={catIndex} className="space-y-3">
                            <h3 className="font-bold text-sm text-gray-800 pb-2 border-b-2 border-purple-200">
                              {category.label}
                            </h3>
                            {category.items.map((subItem, subIndex) => (
                              <a
                                key={subIndex}
                                href={subItem.href}
                                className="block px-3 py-2.5 text-sm text-gray-700 hover:text-purple-600 hover:bg-purple-50 rounded-xl transition-all"
                              >
                                {subItem.label}
                              </a>
                            ))}
